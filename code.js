...
document.getElementById("start-btn").addEventListener("click", function () {
    if (!started) {
      ...
      started = true;
      levelUp(); // Fix: Change this to levelUp
    }
});
...
function checkAns(idx) {
  if (userseq[idx] === gameseq[idx]) {
    ...
    h2.innerHTML = `Game Over! Your Score is <b>${level}</b> <br> Press the button to Start again`; // Fix: Use backticks
...
function highscore() {
  if (level > highestScore) {
    highestScore = level;
    h3.innerText = `High Score: ${highestScore}`; // Assuming you want to display the high score in h3
  }
}
